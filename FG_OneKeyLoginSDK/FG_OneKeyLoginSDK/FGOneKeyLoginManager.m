//
//  FGOneKeyLoginManager.m
//  FG_OneKeyLoginSDK
//
//  Created by zhou on 2024/5/13.
//

#import "FGOneKeyLoginManager.h"
#import "FGCommon.h"

#define ApiHost @"https://api.4321.sh"


@implementation FGOneKeyLoginManager

/// 设置点击协议代理
/// @param delegate 代理
+ (void)setCLFGSDKManagerDelegate:(id<FGOneKeyLoginManagerDelegate>)delegate{
    
    FGOneKeyLoginManager *manager = [[FGOneKeyLoginManager alloc] init];
    manager.delegate = delegate;
    [CLShanYanSDKManager setCLShanYanSDKManagerDelegate:manager];
}
...
+(void)initWithAppId:(NSString *)appId
    ...
    [FGCommon sharedInstance].fgAppId = appId;
    [FGCommon sharedInstance].fgApiKey = apiKey;
    [FGCommon sharedInstance].fgSecret = secret;
    
    // 创建一个网络路径
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@/oneclick/appinfo",ApiHost]];
        ...
        if (httpResponse && httpResponse.statusCode >= 200 && httpResponse.statusCode < 300) {
            ...
            if (jsonError) {
                ...
                if ([json[@"code"] intValue] == 0){
                    [FGCommon sharedInstance].appId = json[@"data"][@"ios_app_id"];
                    [FGCommon sharedInstance].appKey = json[@"data"][@"ios_app_key"];
                    NSLog(@"appId = %@",[FGCommon sharedInstance].appId);
                    // 初始化第三方SDK
                    [CLShanYanSDKManager initWithAppId:[FGCommon sharedInstance].appId complete:^(CLCompleteResult * _Nonnull completeResult) {
                        if (completeResult.error != nil){
                            NSLog(@"初始化失败%@",completeResult.error);
                        }else{
                            NSLog(@"初始化成功");
                        }
                    }];
...
+ (void)setPreGetPhonenumberUseCacheIfNoCellularNetwork:(BOOL)isUseCache{
    [CLShanYanSDKManager setPreGetPhonenumberUseCacheIfNoCellularNetwork:isUseCache];
}
...
+ (void)preGetPhonenumber:(nullable CLComplete)complete{
    [CLShanYanSDKManager preGetPhonenumber:complete];
}
...
+ (void)quickAuthLoginWithConfigure:(CLUIConfigure *)clUIConfigure
    ...
    [CLShanYanSDKManager quickAuthLoginWithConfigure:clUIConfigure complete:complete];
}
...
+ (void)quickAuthLoginWithConfigure:(CLUIConfigure *)clUIConfigure
    ...
    [CLShanYanSDKManager quickAuthLoginWithConfigure:clUIConfigure openLoginAuthListener:openLoginAuthListener oneKeyLoginListener:oneKeyLoginListener];
}

/// 置换手机号
/// @param token    token
/// @param complete         回调block
+ (void)getPhonenumber:(NSString *)token complete:(nonnull CLComplete)complete{
    
    // 创建一个网络路径
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@/oneclick/getmobile",ApiHost]];
    ...
    // 设置请求参数
    NSDictionary *parameters = @{
        @"apikey": [FGCommon sharedInstance].fgApiKey,
        @"app_id": [FGCommon sharedInstance].fgAppId,
        @"secret": [FGCommon sharedInstance].fgSecret,
        @"channel_token": token,
        @"type": @"0",
        @"system": @"0"
    };
    ...
    NSURLSessionDataTask *task = [session dataTaskWithRequest:request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        ...
        if (httpResponse && httpResponse.statusCode >= 200 && httpResponse.statusCode < 300) {
            ...
            if (jsonError) {
                ...
                CLCompleteResult *result = [[CLCompleteResult alloc] init];
                result.data = json;
                complete(result);
...
+ (void)finishAuthControllerAnimated:(BOOL)flag
    ...
    [CLShanYanSDKManager finishAuthControllerAnimated:flag Completion:completion];
}

/// 关闭授权页
/// @param completion       关闭回调（⚠️在子线程中回调）
+ (void)finishAuthControllerCompletion:(void(^_Nullable)(void))completion{
    [CLShanYanSDKManager finishAuthControllerCompletion:completion];
}

/// 返回授权页内置导航控制器（生命周期与授权页一致）
+ (UINavigationController *)authNavigationController{
    return [CLShanYanSDKManager authNavigationController];
}

/// 设置checkBox勾选状态
/// @param isSelect         勾选状态：YES：为勾选
+ (void)setCheckBoxValue:(BOOL)isSelect{
    [CLShanYanSDKManager setCheckBoxValue:isSelect];
}
...
+ (void)mobileCheckWithLocalPhoneNumberComplete:(CLComplete)complete{
    [CLShanYanSDKManager mobileCheckWithLocalPhoneNumberComplete:complete];
}
...
+ (NSString *)getOperatorType{
    return [CLShanYanSDKManager getOperatorType];
}
...
- (void)clShanYanSDKManagerAuthPageAfterViewDidLoad:(UIView *_Nonnull)authPageView
    ...
    [self.delegate clFGSDKManagerAuthPageAfterViewDidLoad:authPageView currentTelecom:telecom];
...
-   (void)clShanYanSDKManagerAuthPageCompleteViewDidLoad:(UIViewController *_Nonnull)authPageVC
    ...
    [self.delegate clFGSDKManagerAuthPageCompleteViewDidLoad:authPageVC currentTelecom:telecom object:object userInfo:userInfo];
...
- (void)clShanYanSDKManagerAuthPageCompleteViewWillAppear:(UIViewController *_Nonnull)authPageVC
                                           currentTelecom:(NSString *_Nullable)telecom
                                                   object:(NSObject *_Nullable)object
                                                 userInfo:(NSDictionary *_Nullable)userInfo{
    [self.delegate clFGSDKManagerAuthPageCompleteViewWillAppear:authPageVC currentTelecom:telecom object:object userInfo:userInfo];
}
...
- (void)clShanYanSDKManagerAuthPageCompleteInit:(UIViewController *_Nonnull)authPageVC
                                 currentTelecom:(NSString *_Nullable)telecom
                                         object:(NSObject*_Nullable)object
                                       userInfo:(NSDictionary*_Nullable)userInfo{
    [self.delegate clFGSDKManagerAuthPageCompleteInit:authPageVC currentTelecom:telecom object:object userInfo:userInfo];
}
...
- (void)clShanYanSDKManagerAuthPageWillPresent:(UIViewController *_Nonnull)authPageVC
                                currentTelecom:(NSString *_Nullable)telecom
                                        object:(NSObject *_Nullable)object
                                      userInfo:(NSDictionary *_Nullable)userInfo{
    [self.delegate clFGSDKManagerAuthPageWillPresent:authPageVC currentTelecom:telecom object:object userInfo:userInfo];
}
...
- (void)clShanYanActionListener:(NSInteger)type
    ...
    [self.delegate clFGActionListener:type code:code message:message];
}
...
- (void)clShanYanPrivacyListener:(NSString *_Nonnull)privacyName
    ...
    [self.delegate clFGPrivacyListener:privacyName privacyURL:URLString authPage:authPageVC];
}
